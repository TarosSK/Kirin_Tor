...
int WorldSocket::HandleAuthSession (WorldPacket& recvPacket)
{
	...
	// VIP
	result = loginDatabase.PQuery("SELECT level FROM account_vip where start<now() and end>now() and realmid = '%u' and acctid='%u'", realmID,id);
    if(result)
    {
        m_Session->SetVIP((*result)[0].GetUInt32());
        delete result;
    }
	else m_Session->SetVIP(0);
